...
";
        ...
        private static int Solve(string input, Func<int[], int> processRow)
        {
            var xs = ParseInput(input);
            return xs.Select(processRow).Sum();
        }

        public static int Solve1(string input = Input)
        {
            return Solve(input, row => row.Max() - row.Min());
        }
        
        public static int Solve2(string input = Input)
        {
            return Solve(input, row =>
            {
